// herald-sol Lambda handler (Function URL, AWS_IAM transport auth).
...
// -> app auth (Supabase JWT | mode='scheduled' system actor | IAM peer) -> chain
// rules -> task -> herald_events + herald_runs. Sol is DETERMINISTIC (no model
// spend); it generates entity data + coverage + SERP scorecards and never posts.
import { randomUUID } from 'node:crypto';
import { authorize } from './auth.mjs';
import { makeDb } from './db.mjs';
import { entityGraphTask, coverageAuditTask, serpAuditTask, knowledgePanelTask } from './tasks.mjs';
import { ALLOWED_CALLERS, MAX_CHAIN_DEPTH, SERVICE, AGENT } from './config.mjs';

export const MANIFEST = {
  service: 'herald-sol', agent: 'sol', engine: 'herald', version: '1.0.0',
  capabilities: ['entity_graph', 'coverage_audit', 'serp_audit', 'knowledge_panel'],
  ...
  generate_not_submit: true,
};

const TASKS = { entity_graph: entityGraphTask, coverage_audit: coverageAuditTask, serp_audit: serpAuditTask, knowledge_panel: knowledgePanelTask };
...
export function createHandler(deps = {}) {
  ...
  return async function handler(event) {
    ...
    // 1) Kill-switch FIRST, fail closed (SPEC-H 5). Sol does not publish, but the
    //    coverage/entity data it emits feeds published surfaces, so on kill/pause
    //    OR unreachable controls it refuses to run - the engine is one system.
    let db;
    ...
    // 2) App auth: Supabase JWT (dashboard) | scheduled system actor | IAM peer.
    let actor;
    ...
    try {
      await db.insertEvent({
        agent: AGENT, event_type: out.eventType || `sol.${task}.${status}`,
