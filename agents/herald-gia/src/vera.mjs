// Peer edge Gia -> Vera (SPEC-H section 4). Gia invokes Vera's Lambda directly
// (lambda:InvokeFunction, granted by invoke_peers = ["vera"] in infra/envs/
// herald-gia). Vera's handler accepts a direct (non-HTTP) event as the envelope
// and proves the caller by IAM + ALLOWED_CALLERS (chain last element = 'gia') -
// no JWT on peer calls (SPEC-B 7.2 rule 4). @aws-sdk/client-lambda ships in the
// nodejs20.x runtime, imported lazily so tests never touch AWS (mock via
// deps.callVera). Gia routes comment copy AND company-page drafts through Vera's
// review_content before either is publishable.
