// herald-gia Lambda handler (Function URL, AWS_IAM transport auth).
// Routes:
...
export function createHandler(deps = {}) {
  ...
  return async function handler(event) {
    ...
    // 1) Kill-switch FIRST, fail closed (SPEC-H 5). Gia proposes engagement and
    //    company-page publishes; on kill/pause OR unreachable controls it refuses -
    //    an engagement Gia clears "leads to a platform action" (SPEC-H 5.4).
    let db;
