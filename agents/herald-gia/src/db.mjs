// Supabase REST + RPC access (service role, herald/gia/cf-service-key). Every
// material action emits a herald_events row (AGT-3 - the audit trail IS the
// conversation). Gia AUTHORS herald_engagement_queue rows (executed / cleared /
// human-tap) and herald_content_queue rows (company-page drafts). The rate
// counters are reserved/committed/rolled-back through the same SECURITY DEFINER
// RPCs Piper uses (migration 0003) so the check-and-increment is atomic (SPEC-D
// 7.1 reserve-then-act). Cookie/session values are NEVER read or written here -
// Gia is a Lambda that reads only the OAuth token vault (vault.mjs).
...
export async function makeDb(fetchImpl = fetch) {
  ...
  return {
    ...
    // Company-page drafts land in the content queue (Gia authors the row).
    ...
    // Engagement queue: executed/cleared warm reactions+comments + human-tap follows.
    insertEngagement: async (row) => (await post('herald_engagement_queue?select=id', row, 'return=representation'))[0],
    patchEngagement: (id, row) => patch(`herald_engagement_queue?id=eq.${id}`, row),
