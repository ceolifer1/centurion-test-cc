...
// conversation). Gia AUTHORS herald_engagement_queue rows (cleared engagement +
// human-tap follows) and herald_content_queue rows (company-page drafts). The
// rate counters are reserved/committed/rolled-back through the same SECURITY
// DEFINER RPCs Piper uses (migration 0003) so the check-and-increment is atomic
// (SPEC-D 7.1 reserve-then-act). Cookie/session values are NEVER read or written
// here - Gia is a Lambda proposer and never touches the vault.
...
export async function makeDb(fetchImpl = fetch) {
  ...
  return {
    ...
    // Engagement queue: cleared warm engagement (like/comment) + human-tap follows.
