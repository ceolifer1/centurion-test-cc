// Kill-switch check (SPEC-H section 5), fail-closed. Read global + agent:linkedin
// + user:<person>; most restrictive wins (kill > pause > run). In-memory cache,
// 60s max TTL (5.2) - no cached 'run' older than 60s authorizes a publish. If CF
// prod is unreachable the checker returns action 'refuse' (5.4): the run may do
// read-only work but MUST NOT publish. Token material is never read here.
