// Supabase REST + RPC access (service role, herald/linkedin/cf-service-key).
// Every material action emits a herald_events row (AGT-3). Rate counters are
// reserved/committed/rolled-back through the SECURITY DEFINER RPCs (migration
// 0003) so check-and-increment is atomic (SPEC-D 7.1 reserve-then-act). Token
// values are NEVER written here (SPEC-C T3).
...
export async function makeDb(fetchImpl = fetch) {
  ...
  return {
    ...
    // Due scheduled LinkedIn items (the schedule sweep publishes each one now -
    // LinkedIn has no scheduling endpoint, so HERALD schedules + posts on wake).
    listDue: (nowIso, limit = 10) =>
      get(`herald_content_queue?status=eq.scheduled&platform=like.linkedin%25&scheduled_for=lte.${encodeURIComponent(nowIso)}&select=*&order=scheduled_for.asc&limit=${limit}`),
