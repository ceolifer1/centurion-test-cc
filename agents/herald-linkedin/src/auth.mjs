...
// per SEC-3): verify the Supabase user token via GET /auth/v1/user, then gate on
// the 'herald' capability in ecosystem_user_grants. Peer/scheduled calls carry
// NO JWT - their identity is proven by AWS_IAM on the Function URL / scheduler
// role and checked in the handler (SPEC-B 7.2).
...
export async function authorize(bearer, fetchImpl = fetch) {
  ...
  const capsArr = Array.isArray(g?.capabilities) ? g.capabilities : [];
  if (!g || !g.is_active || !capsArr.includes(CAPABILITY)) fail(403, `${CAPABILITY} capability required`);
