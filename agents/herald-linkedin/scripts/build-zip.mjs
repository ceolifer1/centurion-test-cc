// Builds dist/function.zip for the herald-nico Lambda (SPEC-B section 3 zip
// path: runtime nodejs20.x, handler src/index.handler). Nico has ZERO runtime
// npm deps (@aws-sdk clients ship in the Lambda runtime), so the zip is just
// package.json + src/. The zip writer is hand-rolled on node:zlib so the build
// itself needs no devDependencies either (identical to herald-vera's builder).
...
// Fixed DOS date/time (2026-07-11 00:00) - deterministic builds.
