// Builds dist/function.zip for the herald-cass Lambda (SPEC-B section 3: runtime
// nodejs20.x, handler src/index.handler). Cass has ZERO runtime npm deps
// (@aws-sdk clients ship in the Lambda runtime), so the zip is package.json +
// src/ (including caps.json). Hand-rolled zip writer on node:zlib so the build
// needs no devDependencies either (identical to the herald-nico/vera builders).
