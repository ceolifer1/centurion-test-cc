...
// per SEC-3 one-agent-one-service): verify the Supabase user token via
// GET /auth/v1/user (CF is legacy HS256, JWKS empty - no local JWT verify), then
// gate on the 'herald' capability in ecosystem_user_grants. Peer-agent calls
// (Cass -> Nico) carry NO JWT - their identity is proven by AWS_IAM on the
// Function URL and checked against ALLOWED_CALLERS in the handler (SPEC-B 7.2).
