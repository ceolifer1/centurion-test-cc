// herald-nico Lambda handler (Function URL, AWS_IAM transport auth).
// Routes:
...
// check FIRST (fail closed) -> app auth (Supabase JWT for the dashboard "draft
// post" action, IAM-proven chain for the Cass peer) -> chain rules -> task ->
// herald_events + herald_runs + agent_usage rows (AGT-3). Nico spends model
// tokens; the ledger row carries the real cost and feeds the engine $400 gate.
import { randomUUID } from 'node:crypto';
import { authorize } from './auth.mjs';
import { makeDb } from './db.mjs';
import { draftPost, redraft } from './tasks.mjs';
import {
...
export const MANIFEST = {
  ...
  engine: 'herald',
  version: '1.0.0',
  capabilities: ['draft_post', 'redraft'],
...
const TASKS = { draft_post: draftPost, redraft };
...
export function createHandler(deps = {}) {
  ...
  return async function handler(event) {
    ...
    // 1) Kill-switch FIRST, fail closed (SPEC-H 5). Nico's work triggers the
    //    Nico->Vera->Piper pipeline that ends in a publish, so on kill/pause OR
    //    unreachable controls Nico refuses - a draft that reaches Vera is a
    //    peer-invoke "that leads to a publish" (SPEC-H 5.4).
    let db;
    ...
    // 4) AGT-3: primary audit event + usage-ledger row (real token cost) + run patch.
    let auditRef = null;
    try {
      await db.insertEvent({
        agent: AGENT,
        event_type: out.eventType || `nico.${task}.${status}`,
    ...
    return respond(httpStatus, {
      ...
      artifacts: out.subjectId
        ? [{ type: out.verdict === 'bounce' ? 'vera_verdict' : 'content_queued', ref: `herald_content_queue:${out.subjectId}`, data: out.bounce || null }]
        : [],
      auditRef,
