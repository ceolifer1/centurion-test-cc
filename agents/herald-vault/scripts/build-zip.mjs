// Builds dist/function.zip for the herald-vault Lambda (SPEC-B section 3 zip path:
// runtime nodejs20.x, handler src/index.handler). Vault has ZERO runtime npm deps
// (@aws-sdk clients ship in the Lambda runtime), so the zip is just package.json
// + src/. The zip writer is hand-rolled on node:zlib so the build needs no
// devDependencies either (identical to herald-nico's builder).
...
const DOS_DATE = u16(((2026 - 1980) << 9) | (7 << 5) | 20);
