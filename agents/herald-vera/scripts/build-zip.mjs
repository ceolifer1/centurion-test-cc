// Builds dist/function.zip for the herald-vera Lambda (SPEC-B section 3 zip
// path: runtime nodejs20.x, handler src/index.handler). Vera has ZERO runtime
// npm deps (@aws-sdk ships in the Lambda runtime), so the zip is just
// package.json + src/ - no node_modules bloat. The zip writer is hand-rolled
// on node:zlib so the build itself needs no devDependencies either.
