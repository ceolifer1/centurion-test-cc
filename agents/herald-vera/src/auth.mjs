// App-level auth for dashboard-origin calls (cf-mandate-ops pattern):
// verify the Supabase user token via GET /auth/v1/user (CF is legacy HS256,
// JWKS empty - no local JWT verify), then gate on the 'herald' capability in
// ecosystem_user_grants. Peer-agent calls carry NO JWT - their identity is
// proven by AWS_IAM on the Function URL and checked against ALLOWED_CALLERS
// in the handler (SPEC-B 7.2 rule 3).
