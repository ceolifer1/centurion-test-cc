...
// The caps are config-as-code (caps.json). Piper obtains a reservation PER ACTION
// before it executes; the counter increments (reserved++) BEFORE the action, so a
// crash can never undercount (SPEC-D 7.1). A would-exceed on ANY window, an
// out-of-active-hours action, a spacing violation, or a disabled action returns
